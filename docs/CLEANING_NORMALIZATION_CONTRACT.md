...
> **Dokumen ini untuk:** Tim Data (Lead, Staff 1, Staff 2) — dibaca sebelum menulis kode apapun di `cleaners/normalize.py` dan `cleaners/merge.py`
> **Sifat:** Kontrak kerja internal — bukan opsional, bukan gaya penulisan bebas. Semua normalizer per sumber (OpenAlex, Semantic Scholar, CrossRef, Google Scholar, dan nanti SINTA/Scopus/GARUDA) **wajib** menghasilkan output yang patuh pada aturan di sini, supaya hasil kerja 3 orang paralel tetap konsisten satu sama lain dan cocok masuk skema `sql/schema_staging.sql`.
> **Rujukan:** `PRD_Data_Engineering_KBK_TI.md` §6 (Data Model, ERD & Vocabulary)
> **Cakupan:** Profil Dosen (`lecturers`), Publikasi (`publications`), Metrik Akademik (`lecturer_metrics`), dan Controlled Vocabulary (`research_clusters`, `research_tags`)
...
2. [Normalisasi Field — Publikasi](#2-normalisasi-field--publikasi)
3. [Normalisasi Field — Profil Dosen](#3-normalisasi-field--profil-dosen)
4. [Normalisasi Field — Metrik Akademik](#4-normalisasi-field--metrik-akademik)
5. [Normalisasi & Tata Kelola — Controlled Vocabulary](#5-normalisasi--tata-kelola--controlled-vocabulary)
6. [Aturan Dedup & Matching](#6-aturan-dedup--matching)
7. [Logging & Audit Trail Wajib](#7-logging--audit-trail-wajib)
8. [Yang Sengaja Tidak Diotomasi di Horizon A](#8-yang-sengaja-tidak-diotomasi-di-horizon-a)
9. [Checklist Sebelum Merge PR Normalizer Baru](#9-checklist-sebelum-merge-pr-normalizer-baru)
...
*Dokumen ini bagian dari `PRD_Data_Engineering_KBK_TI.md` — perubahan pada kontrak ini (terutama aturan dedup §6 dan threshold) wajib dikomunikasikan ke seluruh tim, karena mempengaruhi hasil kerja semua orang, bukan cuma satu fetcher.*
